-- 企业级秒杀 Lua 脚本
-- 原子性完成：活动时间校验 + 库存校验 + 一人一单 + 扣库存 + 已购记录
-- 校验通过后由 Java 生产者发送 RabbitMQ 消息异步落库
--
-- KEYS[1] = 库存 key            seckill:stock:{voucherId}
-- KEYS[2] = 已购用户集合 key    seckill:order:{voucherId}
-- KEYS[3] = 秒杀元数据 key      seckill:voucher:{voucherId}  值格式 "beginEpochSecond,endEpochSecond"
-- ARGV[1] = 用户 ID
-- ARGV[2] = 优惠券 ID
-- ARGV[3] = 当前时间戳（Unix 秒，由 Java 传入，避免跨节点时钟偏差）
--
...
-- ===================== 6. 校验通过，由 Java 生产者发送 RabbitMQ 消息 =====================
